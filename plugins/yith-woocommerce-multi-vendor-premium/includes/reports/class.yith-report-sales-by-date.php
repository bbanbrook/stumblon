	...
	private function query_report_data() {
		...
		foreach ( $commission_ids as $commision_id ) {
			...
			if ( ! isset( $this->report_data->orders_count[ $order_id ] ) ) {
				...
				$this->report_data->orders_count[ $order_id ]->order_date = $order->get_date_created();
			...
			foreach ( $line_items as $line_item_id => $line_item ) {
				...
				if ( in_array( $line_item['product_id'], $products ) ) {
					// net sales
					$this->report_data->orders_net[ $order_id ]->order_date = $order->get_date_created();

					// items purchased
					//$this->report_data->orders_all_product[ $order_id ]              = new stdClass();
					$orders_all_product[ $order_id ] += absint( $line_item['qty'] );
					$this->report_data->orders_all_product[ $order_id ]->order_date = $order->get_date_created();
				}
			}
