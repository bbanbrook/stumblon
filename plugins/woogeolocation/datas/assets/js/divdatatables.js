(function($){
	jQuery.fn.divDataTable = function(ops){
		...
		if (jQuery(self).find('.sort-btn').length <= 0){
			jQuery(self).prepend('<div class="sort-btn"><button class="btn-sort-list" title="View List"></button><button class="btn-sort-grid" title="View Grid"></button></div>');
		}
		...
		jQuery(self).find('div.data-item').each(function(n){
		...
		self.activePage = function(page_active){
			...
			jQuery(self).find('div.data-item').css("display", "none");
